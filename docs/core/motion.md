...
target 是延迟 getter，允许在元素 build 前创建 motion。每次 `enter()` / `leave()`
都会取消仍在进行的旧 Animation，并用当前方向创建新的 Web Animation；`leave()` 会
使用反向 keyframes。内部 Animation 默认 `fill: 'both'`。环境或元素没有 `animate()`
时，enter/leave 立即完成。
