...
```js
import { listen, unlisten, createEventManager } from 'vanilla-jui';
```
...
```
unlisten(target, type, handler, options?) → void
```
...
| `type` | `string` | — | 事件类型 |
| `handler` | `Function \| EventListenerObject` | — | 事件处理器 |
| `options` | `EventListenerOptions \| boolean` | `undefined` | 传递给 `removeEventListener` 的选项 |
...
// ...稍后
unlisten(document, 'click', handleClick);
```
...
class MyComponent {
  ...
  handleResize(e) { console.log('resized'); }
