...
Swiper 是轻量轮播组件，支持触摸/鼠标拖拽、loop、分页、导航、自动播放、图片 lazyload，以及基于 `state.data` 的数据刷新和自动渲染。
...
| 模式     | 入口              | 说明                                              |
| -------- | ----------------- | ------------------------------------------------- |
| 数据模式 | `data`            | 创建新的 Swiper DOM，并支持 `state.data` 自动刷新 |
...
Swiper 会自动清理旧 slide、loop clone、分页、导航绑定和图片加载回调，然后重建视图。
...
绑定模式不会创建根节点，`destroy()` 时也不会移除这个已有根节点。`state.data` 自动重建仅用于数据模式。
...
<div class="j-swiper" data-swiper="root" id="">
  ...
  <div class="swiper-pagination is-horizontal is-clickable is-bullet" data-swiper-pagination></div>
  <button
    type="button"
...
| `data`            | `SwiperDataItem[]` | `[]`     | 初始数据；数据模式下替换后自动重建视图 |
...
| 字段         | 类型               | 说明                                 |
| ------------ | ------------------ | ------------------------------------ |
| `data`       | `SwiperDataItem[]` | 数据源，替换后自动重建 slide         |
