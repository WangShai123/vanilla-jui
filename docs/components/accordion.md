...
const accordion = new Accordion('#demo', {
  ...
  items: [
    ...
    { name: 'usage', title: 'Usage', content: '<strong>Usage</strong> content' },
  ],
});
...
构造器接收 `(container, props)`，`container` 为元素或 CSS 选择器。调用 `render()` 后通过 `vanilla-signal` 的 `insert` 挂载到容器。

## 单开、折叠和多开

```js
// 单开（默认）
new Accordion('#demo', { active: 0, items }).render();

// 折叠 — 点击当前项可关闭
new Accordion('#demo', { active: 0, collapsible: true, items }).render();

// 多开
new Accordion('#demo', { multiple: true, active: [0, 'api'], items }).render();
```
